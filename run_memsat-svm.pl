...
my $memsat_svm_bin_path = $mem_dir.'bin/memsat-svm';
my $globmem_bin_path = $mem_dir.'bin/globmem';
my $mem_pred_svm_bin_path = $mem_dir.'bin/mem_pred';
my $nnsat_bin_path = $mem_dir.'bin/nnsat';
my $mtx = 0;
my $remove_files = 0;
my $globmem = 0;
my $graphics = 1;
...
my $pore_threshold = 0.27;	
my $signal = 1;
					
my %models = ('MEMSAT-SVM_w27_RE.model'=>27,'MEMSAT-SVM_w27_SP.model'=>27,'MEMSAT-SVM_w33_HL.model'=>33,'MEMSAT-SVM_w35_IO.model'=>35,'MEMSAT-SVM_w33_GM.model'=>33,'MEMSAT-SVM_w15_PL.model'=>15);
my $model_s = $model_path."MEMSAT-SVM_STOICH.model";
my @windows = ("15","27","33","35","GM");

my (@mtx,$blast_out,$svm_all,$memsat_out,$memsat_out_single,$stoich_in,$stoich_out,$globmem_out,$png_schematic_out,$png_cartoon_out,$png_cartoon_memsat3_out,%range,%raw_hl,%raw_io,%raw_re,%raw_sp,%raw_pl,%mtx_list,$header,@constraints);
my ($HL_prediction,$IO_prediction,$RE_prediction,$SP_prediction,$GM_prediction,$PL_prediction,$system);
my %topology = ();
...
sub main{
	...
	$PL_prediction = $output_path.$header."_SVM_w15_PL.prediction";
	
	#$system = `rm $input_path/$header*input &> /dev/null` if $remove_files;
	...
	foreach (@mtx){
		...
		$memsat_out = $output_path.$tmp[0].".memsat_svm";
		$globmem_out = $output_path.$tmp[0].".globmem_svm";	
		$svm_all = $output_path.$tmp[0]."_SVM_ALL.out";	
		$stoich_in = $input_path.$tmp[0]."_stoichiometry.input";	
		$stoich_out = $output_path.$tmp[0]."_stoichiometry.out";	
		$png_schematic_out = $output_path.$tmp[0]."_schematic.png";
		$png_cartoon_out = $output_path.$tmp[0]."_cartoon_memsat_svm.png";
		$png_cartoon_memsat3_out = $output_path.$tmp[0]."_cartoon_memsat3.png";

		if (defined $memsat_out_single && scalar @mtx == 1){
			$memsat_out = $output_path.$memsat_out_single.".memsat_svm";
			$png_schematic_out = $output_path.$memsat_out_single."_schematic.png";
			$png_cartoon_out = $output_path.$memsat_out_single."_cartoon_memsat_svm.png";
			$png_cartoon_memsat3_out = $output_path.$memsat_out_single."_cartoon_memsat3.png";			
		}
...
sub get_arguments{
	...
	if (!$ARGV[0]){
		...
		$model_s = $model_path."MEMSAT-SVM_STOICH.model";
...
sub usage {
	...
	print "-w <path>      Directory that contains memsat-svm. Default ''\n";
	...
	print "-t <float>     Pore-lining helix threshold.    Default: 0.27\n";
...
sub run_psiblast {
	...
	my $blast_out = "memsat-svm_tmp.out"; 
	...
	unless (-e $out_mtx || -e $mtx){
		...
		my $system = `cp -f $fasta memsat-svm_tmp.fasta`;
		print "$ncbidir/blastpgp -a $cores -j 2 -h 1e-3 -e 1e-3 -b 0 -d $dbname -i memsat-svm_tmp.fasta -C memsat-svm_tmp.chk >& $blast_out\n\n";
		$system = `$ncbidir/blastpgp -a $cores -j 2 -h 1e-3 -e 1e-3 -b 0 -d $dbname -i memsat-svm_tmp.fasta -C memsat-svm_tmp.chk >& $blast_out`;
		
		unless (-e "memsat-svm_tmp.chk"){
		...
		$system = `echo memsat-svm_tmp.chk > memsat-svm_tmp.pn`;
		$system = `echo memsat-svm_tmp.fasta > memsat-svm_tmp.sn`;
		$system = `echo "$ncbidir/makemat -P memsat-svm_tmp"`;		
		$system = `$ncbidir/makemat -P memsat-svm_tmp`;
		$system = `cp memsat-svm_tmp.mtx $mtx`;
		$system = `rm -f memsat-svm_tmp*`;
...
sub run_memsat {
	...
	if (defined $memsat_out_single && scalar @mtx == 1){
		$tmp[0] = $memsat_out_single;
	}

	my $run_command = "$memsat_svm_bin_path ".$constrained_prediction."-m $helix_score -r $re_helix_score -s $signal -t $pore_threshold -f $format $svm_all > $memsat_out";
	print "$run_command\n";
	...
	if (-e $memsat_out){
		...
		if ($system){
			...
			if ($system){
				...
				$system = `cat $memsat_out | grep "^Pore-lining helices:"`;
				@split = split(/\s+/,$system);
				my @pore_helices = split(/\D+/,$split[2]);
				shift @pore_helices if $format == 3;
				shift @pore_helices if $format == 2;
				my $pore_helices_array = \@pore_helices;
		
				## Predict stoichiometry
				if(scalar @pore_helices){

					my $stoichiometry = 1;

					if(scalar @pore_helices < 6){

						my $f1 = length $seq;
						my $f2 = 0;
						if(scalar @topology_split){
							$f2 = (scalar @topology_split)/2;
						}
						my $f3 = 0;
						$f3 = (scalar @pore_helices)/2 if scalar @pore_helices;
						my $f4 = 0;
						...
						while(my $line = <P>){
							chomp $line;
							my @all_split = split(/\t/,$line);
							push @pore_res_scores,$all_split[5];
						}
						close P;
					
						for(my $j = 0;$j < scalar @topology_split;$j+=2){
							for(my $p = $topology_split[$j];$p <= $topology_split[$j+1];$p++){
								$f4++ if $pore_res_scores[$p-1] > 0;
							}
						}
						open(P,">$stoich_in");
						print P "0 1:$f1 2:$f2 3:$f3 4:$f4\n";
						close P;
					
						print "$svm_classify -v 0 $stoich_in $model_s $stoich_out\n" unless -e $stoich_out;
						my $system = `$svm_classify -v 0 $stoich_in $model_s $stoich_out` unless -e $stoich_out;
						$stoichiometry = `cat $stoich_out`;
						chomp $stoichiometry;
						$stoichiometry = nearest_ceil(1,$stoichiometry);
						$stoichiometry = 1 if $stoichiometry <= 0;
					}

					open(O,">>$memsat_out");
					print O "Pore stoichiometry:\t$stoichiometry\n\n";
					close O;
				}else{
					open(O,">>$memsat_out");
					print O "\n\n";
				...
				unless(`perl -MGD -e 1 &> /dev/stdout`){
					...
					&draw_image($tmp[0],$seq,$topology_array,$pred_n, $re_helix_array,$pred_signal,$pore_helices_array) if $graphics;
...
sub run_memsat3 {
	...
	my $rootname = $output_path.$tmp[0];
	
	my $globmem = $output_path.$tmp[0].".globmem";
	my $nn = $output_path.$tmp[0].".nn";
	my $memsat3 = $output_path.$tmp[0].".memsat3";
	...
	if ($graphics && $runmem3 == 2){
		&draw_image($tmp[0],$seq,\%topology,'',[],0);
		print "Written file $png_schematic_out\n";
		print "Written file $png_cartoon_memsat3_out\n";
	}
}
...
sub parse_predictions {
	...
	if ($globmem != 2){
		...
		unless (open (PL,$PL_prediction)){
			die "Can't open $PL_prediction!\n";
		}
		my @pl = <PL>;
		close PL;
		...
		for($global_counter..($global_counter+$length-1)){
			$pl[$_] =~ s/\s+//g;
			$pl[$_] = nearest_ceil(.001,$pl[$_]);
		}

		#if (scalar @hl != scalar @io || scalar @hl != scalar @re || scalar @re != scalar @sp || scalar @hl != scalar @pl){
		...
		for ($global_counter..($global_counter+$length-1)){
			...
			print RAW "$seq[$seq_counter]\t$hl[$_]\t$io[$_]\t$re[$_]\t$sp[$_]\t$pl[$_]\n";
		
			#print "$seq[$_]\t$hl[$_]\t$io[$_]\t$re[$_]\t$sp[$_]\t$pl[$_]\n";
			
			$raw_hl{$seq_counter+1} = $hl[$_];
			$raw_io{$seq_counter+1} = $io[$_];
			$raw_re{$seq_counter+1} = $re[$_];
			$raw_sp{$seq_counter+1} = $sp[$_];
			$raw_pl{$seq_counter+1} = $pl[$_];
...
sub create_input {
	...
	foreach my $reqwin (@windows){
		...
		for (1..($length + $win - 1)){
			...
			if (($_ <= ($win - 1)/2)||($_ > $length + (($win - 1)/2))){
				...
				if (defined $mtx[$_+13-(($win - 1)/2)]){
					...
					my @aa_20;
					if($reqwin eq "15"){
						@aa_20 = (${$profile{$_}}[1],${$profile{$_}}[3],${$profile{$_}}[4],${$profile{$_}}[5],${$profile{$_}}[6],
						...
						${$profile{$_}}[17],${$profile{$_}}[18],${$profile{$_}}[19],${$profile{$_}}[20],${$profile{$_}}[22]);
					}else{
						@aa_20 = (${$profile{$_}}[1],${$profile{$_}}[3],${$profile{$_}}[4],${$profile{$_}}[5],${$profile{$_}}[6],
						${$profile{$_}}[7],${$profile{$_}}[8],${$profile{$_}}[9],${$profile{$_}}[10],${$profile{$_}}[11],
						${$profile{$_}}[12],${$profile{$_}}[13],${$profile{$_}}[14],${$profile{$_}}[15],${$profile{$_}}[16],
		...
		for my $pos ((($win - 1)/2) + 1..($length + ($win - 1)/2)){
			...
			print IN "0 $vector # ".$real_pos."_".$seq."\n";
		}
...
sub draw_image{
	...
	my ($header,$seq,$topology,$in_out,$re_region,$signal_region,$pore_helices) = @_;
	...
	if (scalar @$pore_helices){
		$topology{'MEMSAT-SVM'}{'pore_lining_helices'} = $pore_helices;
	}
	...
	if ($runmem3 == 2){
		...
		my $im = DrawTransmembraneSchematic->new(-title=>$header,
							 ...
							 -custom_plot_label_7=>'SVM SP/!SP Raw',
						 	 ...
						 	 -draw_custom_plot_9=>1,
						 	 -custom_plot_multiplier_9=>$svm_raw_pl_muliplier,
							 -custom_plot_label_9=>'SVM PL/!PL Raw',
						 	 -custom_plot_data_9=>\%raw_pl,
						 	 -order=>\@order
					         	);
	...
	if (scalar @$pore_helices){
		for (my $r = 0; $r < scalar @$pore_helices; $r += 2){			
			$labels{$$pore_helices[$r+1]} = $$pore_helices[$r]."-".$$pore_helices[$r+1]." Pore-lining";
		}
	}
...
sub get_pl_normalisation_values {
