/* Neural Network Identification of Transmembrane Helices */
/* Copyright (C) 2006 David T. Jones - Last Edited : Jan 2008 */
...
/* Globular/Transmembrane Prediction Module */
                ...
                predict()
{
	...
	if (activation[TOTAL - NUM_OUT] > 0.4)
	{
	    ntmpred++;
	    pred = 'X';
	}
	...
	printf("%c %c %5.3f\n", seq[winpos], pred, activation[TOTAL - NUM_OUT]);
    }
}
...

{
    ...
    /* malloc_debug(3); */
    if (argc != 3)
	fail("usage : globmem weight-file mtx-file");
    randomize();
    init();
    ...
    if (ntmpred)
	printf("\n%d residues predicted to be in TM segments. P(non-TM) = %g\n", ntmpred, 0.1161 * exp(ntmpred * -0.50227416));
    else
	printf("\n%d residues predicted to be in TM segments. P(non-TM) > %g\n", ntmpred, 0.1161 * exp(ntmpred * -0.50227416));
