...
// Pore lining threshold
float pore_threshold = 0.27;

// Containers etc.
float mat[MAXNHEL][MAXSEQLEN];
short length[50][MAXSEQLEN], path[50][MAXSEQLEN];
vector<char> seq;
vector<float> svm_hl,svm_i,svm_o,svm_re,svm_sp,svm_pl;
...
public:
    Topology(vector<int> top_vec, vector<float> pore_vec, const string in_out, vector<float> re_vec, const float scr, int iflg, int seqlen){
			topology_vector = top_vec;
			pore_hel_vector = pore_vec;
...
void Topology::Print(){
	...
	// Search for pore-lining helices
	//cout << "Search for pore-lining helices:" << endl;
	string pore_lining_helix_string;
	int found_pore = 0;
	for (unsigned int h = 0; h < topology_vector.size(); h += 2){
			//cout << 1+(h/2) << ": " << topology_vector[h] << " --- " << topology_vector[h+1];
			float pore_lining_ave = 0;
			unsigned int pore_lining_residues = 0;
			for (int p = topology_vector[h]-1; p <= topology_vector[h+1]-1; p++){
				pore_lining_ave+=svm_pl[p];
				if(svm_pl[p] > 0){
					//cout << p << " score " << svm_pl[p] << endl; 
					pore_lining_residues++;
				}
			}
			pore_lining_ave /= 1+topology_vector[h+1]-topology_vector[h];
			//cout << "\tPore lining: " << pore_lining_ave << " " << pore_lining_residues << endl;
			if(pore_lining_ave >= pore_threshold){
			//if(pore_lining_residues >= (unsigned int)pore_threshold){
				//cout << "\tPore lining: " << pore_threshold << endl;
				...
				if (format == 3){
					...
					if (h != 0){
						...
						if (n_term == "in"){
							if ((h/2) % 2){
								pore_lining_helix_string.append("o");
								suffix.append("i");
							}else{
								pore_lining_helix_string.append("i");
								suffix.append("o");
							}						
						}else{
							if ((h/2) % 2){
					...
					pore_lining_helix_string.append(s1);
					pore_lining_helix_string.append("-");
					pore_lining_helix_string.append(s2);
					pore_lining_helix_string.append(suffix);
				...
				// A.6,21;B.40,57;C.142,172;D.214,236;E.276,302	
					...
					pore_lining_helix_string += letters[h/2];
					pore_lining_helix_string.append(".");
					pore_lining_helix_string.append(s1);
					pore_lining_helix_string.append(",");
					pore_lining_helix_string.append(s2);
				...
				//cout << "\tNot pore lining: " << pore_lining_residues << endl;
			}
	}

	if (re_helix_string == ("")) re_helix_string = ("Not detected.");
	//if (pore_lining_re_helix_string == ("")) pore_lining_re_helix_string = ("Not detected.");
	if (pore_lining_helix_string == ("")) pore_lining_helix_string = ("Not detected.");
		
	cout << "Topology:\t\t" << topology_string << endl;
	cout << "Re-entrant helices:\t" << re_helix_string << endl;
	//cout << "Pore-lining re-entrant helices:\t" << pore_lining_re_helix_string << endl;
	cout << "Pore-lining helices:\t" << pore_lining_helix_string << endl;
...
static void usage(){
	...
	cout << "  -t <int>        Residues required to predict pore-lining helix.    Default: " << pore_threshold << endl;
...
static void parse_input_file(string& file, int & seqlen){
        ...
        while(getline(is,line)){
            ...
            if (sscanf(ch, "%c%f%f%f%f%f", &s, &h, &i, &r, &sig, &pl) != 6){
...
// Generate pore-lining helix score
float porescore(int start, int stop){

	float pore_lining_ave = 0;
	for (int p = start-1; p <= stop-1; p++){
		//printf("%d - %f\n",p,svm_pl[p]);
		pore_lining_ave+=svm_pl[p];

	}
	pore_lining_ave /= 1+stop-start;
	return(pore_lining_ave);
}
...
float trace_back(int starthel, int inflg, int nhelix, vector<Topology>& topologies, unsigned int seqlen){
	...
	vector<int> topology_vector;
	vector<float> re_helix_vector;
	vector<float> pore_score_vector;
	...
	for (h = 0; h < nhelix-starthel; h++, intog = !intog){
		...
		float pore_score = porescore(res+1,res + length[h+starthel][res]);
		// Calculate posterior probability
		float z = (pore_score - 0.0348778440419162)/0.464864759671979;
		float post_prob = 1/(1 + exp((-9.89893 * z) + 4.87813));

		printf("Pore-lining raw score, posterior probability:\t\t%2.3f,%2.3f\n",pore_score,post_prob);
		pore_score_vector.push_back(pore_score);
	...
	topologies.push_back(Topology(topology_vector,pore_score_vector,n_term,re_helix_vector,score,inflg,seqlen));
    return (score);
    ...
    while( i < argc){
        if( argv[i][0] == '-'){
            ...
            switch(argv[i-1][1]){
				...
				case 't' : {pore_threshold = atof(argv[i]); break;}					
